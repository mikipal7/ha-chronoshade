...
# Schema for time-position maps in dict format {time: position}
TIME_POSITION_MAP_SCHEMA = vol.Schema({
...
TRAVEL_TIME_SCHEMA = {
    ...
    vol.Optional(CONF_OPEN_MAP, default=None): TIME_POSITION_MAP_SCHEMA,
    vol.Optional(CONF_CLOSE_MAP, default=None): TIME_POSITION_MAP_SCHEMA,
    vol.Optional(CONF_TILT_OPEN_MAP, default=None): TIME_POSITION_MAP_SCHEMA,
    vol.Optional(CONF_TILT_CLOSE_MAP, default=None): TIME_POSITION_MAP_SCHEMA,
}
...
class TimePositionCalculator:
    """Calculate position based on time using separate open/close position-time maps."""
    
    def __init__(self, open_map=None, close_map=None, travel_time=None):
        """Initialize the calculator."""
        self.open_map = self._convert_dict_to_list(open_map) if open_map else None
        self.close_map = self._convert_dict_to_list(close_map) if close_map else None
        ...
        # Validate maps if provided
        if self.open_map:
            self._validate_map(self.open_map, "open_map")
        if self.close_map:
            self._validate_map(self.close_map, "close_map")
            
    def _convert_dict_to_list(self, time_map_dict):
        """Convert {time: position} dict to list of {time, position} dicts."""
        if not time_map_dict:
            return None
        
        return sorted([
            {"time": float(time), "position": int(position)} 
            for time, position in time_map_dict.items()
        ], key=lambda x: x["time"])
    
    def _validate_map(self, position_time_map, map_name):
        """Validate a position-time map."""
        if len(position_time_map) < 2:
            raise ValueError(f"{map_name} must have at least 2 points")
        
        # Validate first point starts at time 0
        if position_time_map[0]['time'] != 0:
            raise ValueError(f"First time entry in {map_name} must be 0")
            
        # Validate times are in ascending order
        prev_time = -1
        for point in position_time_map:
            if point['time'] <= prev_time:
                raise ValueError(f"Times in {map_name} must be in ascending order")
    ...
    def start_travel(self, target_position, current_position=None):
        ...
        # Select appropriate map based on direction
        if self._direction == TravelStatus.DIRECTION_UP:
            self._current_map = self.open_map
        else:
            self._current_map = self.close_map
    ...
    def update_position(self, now):
        ...
        if self._current_map:
            # Use the selected map for this direction
            self._last_position = self._calculate_position_from_map(elapsed, self._current_map)
        else:
            # Fallback to linear movement based on travel time
            progress = min(1.0, elapsed / self.travel_time) if self.travel_time else 0
    ...
    def _calculate_position_from_map(self, elapsed, position_map):
        """Calculate position from a position-time map."""
        # Find the two points in the map that bracket the elapsed time
        prev_point = None
        next_point = None
        
        for point in position_map:
        ...
        # Calculate relative to start position
        map_start_pos = position_map[0]['position']
        map_end_pos = position_map[-1]['position']
        map_progress = (prev_point['position'] + (position_diff * ratio) - map_start_pos) / (map_end_pos - map_start_pos)
        
        return self._start_position + (self._target_position - self._start_position) * map_progress
...
def devices_from_config(domain_config):
    ...
    for device_id, config in domain_config[CONF_DEVICES].items():
        ...
        # Handle new open/close map configurations
        open_map = config.pop(CONF_OPEN_MAP, None)
        close_map = config.pop(CONF_CLOSE_MAP, None)
        tilt_open_map = config.pop(CONF_TILT_OPEN_MAP, None)
        tilt_close_map = config.pop(CONF_TILT_CLOSE_MAP, None)
        ...
        device = CoverTimeBased(
            ...
            open_map=open_map,
            close_map=close_map,
            tilt_open_map=tilt_open_map,
            tilt_close_map=tilt_close_map,
...
class CoverTimeBased(CoverEntity, RestoreEntity):
    def __init__(
        ...
        self._tilt_open_map = tilt_open_map
        self._tilt_close_map = tilt_close_map
        ...
        if open_map or close_map:
            # Use new TimePositionCalculator with separate open/close maps
            self.travel_calc = TimePositionCalculator(
                open_map=open_map,
                close_map=close_map,
                travel_time=max(travel_time_down or 0, travel_time_up or 0) or DEFAULT_TRAVEL_TIME
            )
        elif position_time_map:
            # Backward compatibility: convert old format to new
            full_travel_time = max(point['time'] for point in position_time_map)
            # For backward compatibility, use the same map for both directions
            old_map_dict = {point['time']: point['position'] for point in position_time_map}
            self.travel_calc = TimePositionCalculator(
                open_map=old_map_dict,
                close_map=old_map_dict,
                travel_time=full_travel_time
            )
            self._travel_time_up = full_travel_time
            self._travel_time_down = full_travel_time
        else:
            # Use original TravelCalculator for linear movement
            self.travel_calc = TravelCalculator(
                self._travel_time_down or DEFAULT_TRAVEL_TIME,
                self._travel_time_up or DEFAULT_TRAVEL_TIME,
            )

        # Initialize tilt calculators
        if self._has_tilt_support():
            if tilt_open_map or tilt_close_map:
                self.tilt_calc = TimePositionCalculator(
                    open_map=tilt_open_map,
                    close_map=tilt_close_map,
                    travel_time=max(tilt_time_down or 0, tilt_time_up or 0) or DEFAULT_TRAVEL_TIME
                )
            elif tilt_position_time_map:
                # Backward compatibility for tilt
                full_tilt_time = max(point['time'] for point in tilt_position_time_map)
                old_tilt_map_dict = {point['time']: point['position'] for point in tilt_position_time_map}
                self.tilt_calc = TimePositionCalculator(
                    open_map=old_tilt_map_dict,
                    close_map=old_tilt_map_dict,
                    travel_time=full_tilt_time
                )
    ...
    def extra_state_attributes(self):
        ...
        if self._open_map is not None:
            attr[CONF_OPEN_MAP] = self._open_map
        if self._close_map is not None:
            attr[CONF_CLOSE_MAP] = self._close_map
        if self._tilt_open_map is not None:
            attr[CONF_TILT_OPEN_MAP] = self._tilt_open_map
        if self._tilt_close_map is not None:
            attr[CONF_TILT_CLOSE_MAP] = self._tilt_close_map
        return attr
